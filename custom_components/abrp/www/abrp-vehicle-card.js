...
`;function je(o){let e=c=>g(o.hass,c),t=o._vs("sensor.data_source")?.attributes?.providers||{},i=(c,h=e("live.estimate"))=>W(t[c])||h,s=(c,h)=>o._vs(c)?.attributes?.unit_of_measurement??h,r=Number(o._vs("sensor.speed_factor")?.state),n=[[e("live.soc"),$(o._vs("sensor.soc")),"%",i("soc"),"sensor.soc"],[e("live.range"),$(o._vs("sensor.range")),s("sensor.range","km"),i("est_battery_range"),"sensor.range"],[e("live.ref_consumption"),$(o._vs("sensor.reference_consumption")),s("sensor.reference_consumption","Wh/km"),i("calib_ref_cons"),"sensor.reference_consumption"],[e("live.capacity"),$(o._vs("sensor.battery_capacity")),"kWh",i("battery_capacity",i("capacity")),"sensor.battery_capacity"],[e("live.odometer"),$(o._vs("sensor.odometer")),s("sensor.odometer","km"),i("odometer"),"sensor.odometer"],[e("live.location"),o._vs("device_tracker.location")?.state,"",i("lat",""),"device_tracker.location"],[e("live.ref_speed"),Number.isFinite(r)?Math.round(r*100):null,"%",i("speed_factor"),"sensor.speed_factor"],[e("live.max_speed"),$(o._vs("sensor.max_speed")),s("sensor.max_speed","km/h"),i("max_speed"),"sensor.max_speed"],[e("live.elevation"),$(o._vs("sensor.elevation")),s("sensor.elevation","m"),i("elevation",""),"sensor.elevation"]],d=W(o._vs("sensor.data_source")?.state),l=c=>c&&c!=="unknown"&&c!=="unavailable",p=new Map;for(let[c,h,_]of[[d,o._vs("sensor.source_last_refresh")?.state,"sensor.source_last_refresh"],["Obdble",o._vs("sensor.obd_last_refresh")?.state,"sensor.obd_last_refresh"]]){if(!c||!l(h))continue;let m=p.get(c.toLowerCase());(!m||new Date(h)>new Date(m[1]))&&p.set(c.toLowerCase(),[c,h,_])}let u=[...p.values()];return a`<div class="grid">
    ...
    ${_t(o,t)}
  `}function pt(o,e){return a`${V("mdi:ev-station",g(o.hass,"options.charge_stops"))}
    <div class="segments">
    ...
    </div>`}function _e(o,e,t,i,s){if(!i)return"";let r=i.attributes,n=Number(i.state);return a`${V(t,e)}
    <div class="slider-row">
      ...
      <ha-slider
        ...
        step=${r.step??1}
        .value=${n}
        @change=${d=>{o._call("number","set_value",i,{value:Number(d.target.value)}),d.target.closest(".slider-row").querySelector(".slider-value").textContent=`${d.target.value}${s}`}}
      ></ha-slider>
  ...
  >
    ...
    ></ha-switch>
  </div>`:""}function _t(o,e){return e?.attributes?.options?.length?a`${V("mdi:car-cog",g(o.hass,"options.drive_profile"))}
    <ha-select
      naturalMenuWidth
      fixedMenuPosition
      .value=${e.state}
    ...
    >
      ...
      </ha-card>`}_status(){let e=Date.now()/1e3,t=h=>{if(!h||h==="unknown"||h==="unavailable")return null;let _=new Date(h).getTime();return Number.isNaN(_)?null:_/1e3},i=this._vs("sensor.data_source"),s=t(i?.attributes?.soc_last_seen),r=Number(this._vs("sensor.soc")?.state),n=Number.isFinite(r)&&s!=null&&e-s<300,d=this._vs("binary_sensor.charging")?.state==="on",l=Math.abs(Number(this._vs("sensor.charging_power")?.state)),p=this._vs("binary_sensor.asleep")?.state==="on",u=i?.attributes?.last_seen??this._vs("sensor.last_update")?.state,c=t(u);if(n&&d&&Number.isFinite(l)&&l>0)return{color:"green",pulse:!0,text:this._t("card.charging")};if(n)return{color:"green",pulse:!0,text:this._t("card.connected")};if(c!=null&&e-c<=3*3600){let h=J(u,this.hass);return{color:"gray",pulse:!1,text:h?this._t("card.last_seen",{time:h}):this._t("card.never_seen")}}return p?{color:"gray",pulse:!1,text:this._t("card.sleeping")}:{color:"gray",pulse:!1,text:this._t("card.offline")}}_renderMain(e){let t=this._vs("sensor.vehicle_name")?.state,i=this._config.title,r=(i&&y(i)?String(this._templateResults?.[i]??""):i)||(t&&t!=="unknown"&&t!=="unavailable"?t:null)||e.device?.name_by_user||e.device?.name||this._t("card.vehicle"),n=this._vs("sensor.soc"),d=$(n),l=this._vs("image.car_image"),p=l?.attributes?.entity_picture||(typeof l?.state=="string"&&(l.state.startsWith("http")||l.state.startsWith("/"))?l.state:null),u=this._status(),c=this._vs("binary_sensor.charging")?.state==="on",h=Number(this._vs("sensor.charging_power")?.state),_=c&&Number.isFinite(h)&&h>0?`${h<10?h.toFixed(1):Math.round(h)} kW`:null,m=M=>this._config[M]!==!1;return a`<div class="main">
      <div class="head">
        <div class="head-left">
          <div class="name">${r}</div>
          ${m("show_profile")?this._renderProfile():""}
        </div>
        ${m("show_image")&&p?a`<img
              class="car clickable"
              src="${p}"
              alt="${r}"
      ...
      <div class="soc-row clickable" @click=${()=>this._moreInfo("sensor.soc")}>
        ...
        ${m("show_charge_speed")&&_?a`<span
            ...
            </span>`:m("show_charge_speed")&&c?a`<span class="charge-speed">
                <ha-icon icon="mdi:flash"></ha-icon>${this._t("card.charging")}
              </span>`:""}
      </div>
      <div class="bar clickable" @click=${()=>this._moreInfo("sensor.soc")}>
        <div class="fill ${c?"charging":""}" style="width:${d??0}%"></div>
      </div>
      ${m("show_last_seen")||m("show_live_data")?a`<div class="meta">
            ${m("show_last_seen")?a`<span
                  class="seen clickable"
            ...
            ${m("show_live_data")?a`<a class="link" @click=${()=>this._dialog="live"}
                  >${this._t("card.live_data")}</a
                >`:""}
          </div>`:""}
      ${m("show_options")||this._config.show_live_data_button===!0?a`<div class="buttons">
            ${m("show_options")?a`<button
                  class="btn"
