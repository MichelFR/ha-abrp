...
`;var ke=3600,Se=86400,J=604800,st=2592e3,Nt={soc:Se,soe:Se,power:300,hvac_power:300,speed:30,capacity:st,kwh_charged:300,soh:st,voltage:300,current:300,odometer:J,est_battery_range:Se,ext_temp:ke,batt_temp:ke,cabin_temp:ke,lat:J,lon:J,heading:J,elevation:J};function ot(o){let e=p=>v(o.hass,p),t=o._vs("sensor.data_source")?.attributes||{},i=t.providers||{},s=t.timestamps||{},r=Date.now()/1e3,n=p=>{if(!p)return!0;let f=Nt[p],x=Number(s[p]);return f==null||!Number.isFinite(x)||x<=0?!0:r-x<=f},l=(p,f=e("live.estimate"))=>L(i[p])||f,c=(p,f)=>o._vs(p)?.attributes?.unit_of_measurement??f,d=(p,f)=>{if(f&&!n(f))return null;let x=Number(o._vs(p)?.state);return Number.isFinite(x)?x:null},u=p=>Math.abs(p)<10?p.toFixed(1):p.toFixed(0),h=d("sensor.power","power"),m=o._vs("binary_sensor.charging")?.state==="on",_=h==null?null:m?-h:h,$=i.power==="derived"&&i.current!=null?i.current:i.power,U=d("sensor.soh","soh"),b=U==null?null:100-U,F=d("sensor.calibration_confidence")==null?null:N(o._vs("sensor.reference_consumption")),Ce=N(o._vs("sensor.calibrated_max_speed")),dt=Ce??N(o._vs("sensor.max_speed")),ht=Ce!=null?l("calib_max_speed"):l("max_speed"),Q=o._vs("device_tracker.location"),pt=Q?.attributes?.address,ut=n("lat")?pt||(Q?.state&&Q.state!=="unknown"?Q.state:null):null,Pe=d("sensor.speed_factor"),ee=o._vs("sensor.firmware_version")?.state,ze=d("sensor.soc","soc"),Oe=d("sensor.hvac_power","hvac_power"),mt=[[e("live.soc"),ze==null?null:ze.toFixed(0),"%",l("soc"),"sensor.soc"],[e("live.power"),_==null?null:u(_),"kW",L($)||e("live.estimate"),"sensor.power"],[e("live.hvac_power"),Oe==null?null:u(Oe),"kW",l("hvac_power"),"sensor.hvac_power"],[e("live.range"),d("sensor.range","est_battery_range")?.toFixed(0)??null,c("sensor.range","km"),l("est_battery_range"),"sensor.range"],[e("live.voltage"),d("sensor.voltage","voltage")?.toFixed(0)??null,"V",l("voltage"),"sensor.voltage"],[e("live.ref_consumption"),F,c("sensor.reference_consumption","Wh/km"),l("calib_ref_cons"),"sensor.reference_consumption"],[e("live.batt_temp"),d("sensor.battery_temp","batt_temp")?.toFixed(0)??null,c("sensor.battery_temp","\xB0C"),l("batt_temp"),"sensor.battery_temp"],[e("live.degradation"),b==null?null:u(b),"%",l("soh"),"sensor.soh"],[e("live.capacity"),d("sensor.battery_capacity","capacity")?.toFixed(0)??null,"kWh",l("battery_capacity",l("capacity")),"sensor.battery_capacity"],[e("live.ref_speed"),Pe==null?null:Math.round(Pe*100),"%",l("speed_factor"),"sensor.speed_factor"],[e("live.max_speed"),dt,c("sensor.calibrated_max_speed","km/h"),ht,"sensor.calibrated_max_speed"],[e("live.soe"),d("sensor.soe","soe")?.toFixed(1)??null,"kWh",l("soe"),"sensor.soe"],[e("live.inside_temp"),d("sensor.cabin_temp","cabin_temp")?.toFixed(0)??null,c("sensor.cabin_temp","\xB0C"),l("cabin_temp"),"sensor.cabin_temp"],[e("live.outside_temp"),d("sensor.external_temp","ext_temp")?.toFixed(0)??null,c("sensor.external_temp","\xB0C"),l("ext_temp"),"sensor.external_temp"],[e("live.odometer"),d("sensor.odometer","odometer")?.toFixed(0)??null,c("sensor.odometer","km"),l("odometer"),"sensor.odometer"],[e("live.location"),ut,"",l("lat",""),"device_tracker.location"],[e("live.elevation"),d("sensor.elevation","elevation")?.toFixed(0)??null,c("sensor.elevation","m"),l("elevation",""),"sensor.elevation"],[e("live.firmware"),ee&&ee!=="unknown"&&ee!=="unavailable"?ee:null,"","","sensor.firmware_version"]].filter(([,p])=>p!=null),_t=L(o._vs("sensor.data_source")?.state),gt=p=>p&&p!=="unknown"&&p!=="unavailable",de=new Map;for(let[p,f,x]of[[_t,o._vs("sensor.source_last_refresh")?.state,"sensor.source_last_refresh"],["Obdble",o._vs("sensor.obd_last_refresh")?.state,"sensor.obd_last_refresh"]]){if(!p||!gt(f))continue;let H=de.get(p.toLowerCase());(!H||new Date(f)>new Date(H[1]))&&de.set(p.toLowerCase(),[p,f,x])}let Re=[...de.values()];return a`<div class="grid">
    ...
    ${Re.length?a`<div class="sources">
          ${Re.map(([p,f,x])=>a`<span
            ...
            >
              ...
              <span class="src-time">${ne(f,o.hass)}</span>
            </span>`)}
        </div>`:""}`}var rt={optimal:"options.optimal",fewer:"options.fewer",least:"options.least"},Dt=[["switch.avoid_tolls","mdi:cash-multiple","options.tolls"],["switch.avoid_motorways","mdi:highway","options.highways"],["switch.avoid_ferries","mdi:ferry","options.ferries"],["switch.avoid_borders","mdi:boom-gate","options.borders"]],Ut=[["switch.realtime_traffic","mdi:traffic-light","options.traffic"],["switch.realtime_weather","mdi:weather-partly-cloudy","options.weather"],["switch.adjust_speed","mdi:speedometer","options.adjust_speed"]],X=(o,e)=>a`<div class="section"><ha-icon icon=${o}></ha-icon>${e}</div>`;function nt(o){let e=o._as("select.charge_stops"),t=o._vs("select.drive_profile"),i=s=>v(o.hass,s);return a`
    ${e?Ft(o,e):""}
    ${Ee(o,i("options.arrival_soc"),"mdi:battery-low",o._as("number.arrival_soc"),"%")}
    ${X("mdi:cancel",i("options.avoid"))}
    <div class="chips">
      ${Dt.map(([s,r,n])=>Ht(o,s,r,i(n)))}
    </div>
    ${X("mdi:update",i("options.realtime"))}
    ${Ut.map(([s,r,n])=>Bt(o,s,r,i(n)))}
  ...
  `}function Ft(o,e){return a`${X("mdi:ev-station",v(o.hass,"options.charge_stops"))}
    <div class="segments">
      ${(e.attributes.options||[]).map(t=>a`<button
        ...
        >
          ${rt[t]?v(o.hass,rt[t]):L(t)}
        </button>`)}
    </div>`}function Ee(o,e,t,i,s){if(!i)return"";let r=i.attributes,n=Number(i.state);return a`${X(t,e)}
    <div class="slider-row">
      ...
      ></ha-slider>
    </div>`}function Ht(o,e,t,i){let s=o._as(e);if(!s)return"";let r=s.state==="on";return a`<button
    class="chip ${r?"on":""}"
    @click=${()=>o._call("switch","toggle",s)}
  >
    <ha-icon icon="${t}"></ha-icon>${i}
  </button>`}function Bt(o,e,t,i){let s=o._as(e);return s?a`<div class="switch-row">
    <span class="switch-label"><ha-icon icon=${t}></ha-icon>${i}</span>
    <ha-switch
      .checked=${s.state==="on"}
      @change=${()=>o._call("switch","toggle",s)}
    ></ha-switch>
  </div>`:""}function jt(o,e){return e?.attributes?.options?.length?a`${X("mdi:car-cog",v(o.hass,"options.drive_profile"))}
    <ha-select
      naturalMenuWidth
      fixedMenuPosition
      .value=${e.state}
    ...
    >
      ...
      </ha-card>`}_status(){let e=Date.now()/1e3,t=m=>{if(!m||m==="unknown"||m==="unavailable")return null;let _=new Date(m).getTime();return Number.isNaN(_)?null:_/1e3},i=this._vs("sensor.data_source"),s=t(i?.attributes?.soc_last_seen),r=Number(this._vs("sensor.soc")?.state),n=Number.isFinite(r)&&s!=null&&e-s<300,l=this._vs("binary_sensor.charging")?.state==="on",c=Math.abs(Number(this._vs("sensor.charging_power")?.state)),d=this._vs("binary_sensor.asleep")?.state==="on",u=i?.attributes?.last_seen??this._vs("sensor.last_update")?.state,h=t(u);if(n&&l&&Number.isFinite(c)&&c>0)return{color:"green",pulse:!0,text:this._t("card.charging")};if(n)return{color:"green",pulse:!0,text:this._t("card.connected")};if(h!=null&&e-h<=3*3600){let m=ne(u,this.hass);return{color:"gray",pulse:!1,text:m?this._t("card.last_seen",{time:m}):this._t("card.never_seen")}}return d?{color:"gray",pulse:!1,text:this._t("card.sleeping")}:{color:"gray",pulse:!1,text:this._t("card.offline")}}_renderMain(e){let t=this._vs("sensor.vehicle_name")?.state,i=this._config.title,r=(i&&k(i)?String(this._templateResults?.[i]??""):i)||(t&&t!=="unknown"&&t!=="unavailable"?t:null)||e.device?.name_by_user||e.device?.name||this._t("card.vehicle"),n=this._vs("sensor.soc"),l=Number(this._vs("sensor.data_source")?.attributes?.timestamps?.soc),d=Number.isFinite(l)&&l>0&&Date.now()/1e3-l>86400?null:N(n),u=this._vs("image.car_image"),h=u?.attributes?.entity_picture||(typeof u?.state=="string"&&(u.state.startsWith("http")||u.state.startsWith("/"))?u.state:null),m=this._status(),_=this._vs("binary_sensor.charging")?.state==="on",$=Number(this._vs("sensor.charging_power")?.state),U=_&&Number.isFinite($)&&$>0?`${$<10?$.toFixed(1):Math.round($)} kW`:null,b=F=>this._config[F]!==!1;return a`<div class="main">
      <div class="head">
        <div class="head-left">
    ...
    >
    ...
    </ha-dialog>`}};var It=[{name:"device",selector:{device:{integration:B,entity:[{integration:B,domain:"device_tracker"}]}}}],at={illustration:[["show_image",!0,"mdi:image-outline"]],profile:[["show_profile",!0,"mdi:car-cog"],["confirm_profile_change",!0,"mdi:shield-check-outline"]],battery:[["show_charge_speed",!0,"mdi:flash"]],status:[["show_last_seen",!0,"mdi:clock-outline"],["show_live_data",!0,"mdi:link-variant"]],buttons:[["show_options",!0,"mdi:tune-variant"],["show_live_data_button",!1,"mdi:chart-box-outline"]]},lt={illustration:[["image.car_image","mdi:image-outline"]],profile:[["select.drive_profile","mdi:car-cog"]],battery:[["sensor.soc","mdi:battery-high"],["binary_sensor.charging","mdi:battery-charging"],["sensor.charging_power","mdi:flash"]],status:[["sensor.last_update","mdi:clock-outline"]],livedata:[["sensor.soc","mdi:battery-high"],["sensor.range","mdi:map-marker-distance"],["sensor.reference_consumption","mdi:lightning-bolt-outline"],["sensor.battery_capacity","mdi:battery"],["sensor.odometer","mdi:counter"],["device_tracker.location","mdi:map-marker"],["sensor.speed_factor","mdi:speedometer"],["sensor.max_speed","mdi:speedometer-medium"],["sensor.elevation","mdi:image-filter-hdr"],["sensor.data_source","mdi:database-outline"],["sensor.source_last_refresh","mdi:cloud-outline"],["sensor.obd_last_refresh","mdi:car-connected"]]},ct=[{id:"title",icon:"mdi:format-title"},{id:"illustration",icon:"mdi:image-outline"},{id:"profile",icon:"mdi:car-cog"},{id:"battery",icon:"mdi:battery-charging"},{id:"status",icon:"mdi:clock-outline"},{id:"buttons",icon:"mdi:gesture-tap-button"},{id:"livedata",icon:"mdi:chart-box-outline"}],D="sensor.vehicle_name",ce=class extends y{static get properties(){return{hass:{},_config:{},_page:{state:!0},_modes:{state:!0}}}constructor(){super(),this._page=null,this._modes={}}connectedCallback(){super.connectedCallback(),ae()}setConfig(e){this._config=e||{}}_t(e,t){return v(this.hass,e,t)}render(){if(!this.hass)return a``;let e=ct.find(t=>t.id===this._page);return e?this._renderSubpage(e):this._renderRoot()}_defaults(){let e=Z(this.hass),t=this._config.device&&e.find(i=>i.deviceId===this._config.device)||e[0];return{...t?M(this.hass,t.ents):{},...M(this.hass,re(this.hass))}}_renderRoot(){return a`<ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${It}
      ...
      </div>`}_summary(e){if(e==="title")return this._config.title||(this._config.entities?.[D]??this._t("editor.auto_name"));let t=(lt[e]||[]).filter(([n])=>this._config.entities?.[n]).length,i=t?` \xB7 ${this._t("editor.overridden",{n:t})}`:"",s=at[e]||[];if(!s.length)return t?this._t("editor.overridden",{n:t}):this._t("editor.automatic");let r=s.filter(([n,l])=>this._config[n]??l);return r.length?r.length===s.length&&s.length===1?`${this._t("editor.shown")}${i}`:r.map(([n])=>this._t(`short.${n}`)).join(", ")+i:`${this._t("editor.nothing_shown")}${i}`}_renderSubpage(e){return a`<div class="subpage-head">
      ...
      ${e.id==="title"?this._renderTitleSlot():(lt[e.id]||[]).map(([t,i])=>this._renderSlot(t,i))}`}_renderToggle(e,t,i){return a`<div class="row">
      ...
      ${this._renderModeChips("__title",i,r=>{this._modes={...this._modes,__title:r},r==="auto"?this._dispatch(this._withoutTitle(this._withOverride(D,""))):r==="entity"?this._dispatch(this._withoutTitle(this._config)):r==="custom"&&this._dispatch(this._withOverride(D,""))})}
      ${i==="auto"?a`<div class="hint">
            ${this._t("editor.auto_value",{value:s||this._t("editor.auto_name")})}
              ...
              .schema=${[{name:"value",selector:{entity:{}}}]}
              .computeLabel=${()=>this._t("editor.entity")}
              @value-changed=${r=>{r.stopPropagation(),this._setOverride(D,r.detail.value.value||"")}}
            ></ha-form>`:a`<ha-form
              .hass=${this.hass}
              .data=${{value:t}}
              .schema=${[{name:"value",selector:{template:{}}}]}
              .computeLabel=${()=>this._t("editor.custom_name")}
              @value-changed=${r=>{r.stopPropagation();let n={...this._config,type:`custom:${w}`};r.detail.value.value?n.title=r.detail.value.value:delete n.title,this._dispatch(n)}}
    ...
    `}};customElements.define(w,le);customElements.define(`${w}-editor`,ce);window.customCards=window.customCards||[];window.customCards.push({type:w,name:"ABRP Vehicle Card",description:"ABRP-style vehicle card with battery state, live data and plan options.",preview:!0,documentationURL:"https://github.com/MichelFR/ha-abrp"});
