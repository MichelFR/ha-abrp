...
export function renderLiveData(card) {
  ...
  const tiles = [
    ["SoC", num(card._vs("sensor.soc")), "%", provider("soc")],
    [
      "Range",
  ...
  const sources = [
    [cloudName, card._vs("sensor.source_last_refresh")?.state],
    ["Obdble", card._vs("sensor.obd_last_refresh")?.state],
  ].filter(([n, t]) => n && t && t !== "unknown" && t !== "unavailable");

  return html`<div class="grid">
      ${tiles.map(
        ([title, value, unitLabel, prov]) => html`<div class="tile">
